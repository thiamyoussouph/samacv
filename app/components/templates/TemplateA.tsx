...
const TemplateA = forwardRef<HTMLDivElement, Props>(({ personalDetails, file, theme, experiences, educations, languages, skills, hobbies, download, extraPages = 0 }, ref) => {
  return (
    <div ref={ref} className="w-full flex flex-col gap-10">
      {/* PAGE 1 */}
      <div className={`page flex p-16 w-[950px] min-h-[1340px] shadow-lg ${download ? 'mb-10' : ''}`} data-theme={theme}>
        {/* Colonne gauche */}
              ...
              <ul className="space-y-2">
                <li className="flex">
                  <div className="break-all text-sm relative">
                    ...
                    {personalDetails.phone && <div className="absolute left-0 top-0"><Phone className="w-5 text-primary" /></div>}
                  </div>
                </li>
                <li className="flex">
                  <div className="break-all text-sm relative">
                    <div className="ml-8">{personalDetails.email}</div>
                    {personalDetails.email && <div className="absolute left-0 top-0"><Mail className="w-5 text-primary" /></div>}
                  </div>
                </li>
                <li className="flex">
                  <div className="break-all text-sm relative">
                    <div className="ml-8">{personalDetails.address}</div>
                    {personalDetails.address && <div className="absolute left-0 top-0"><MapPinCheckInside className="w-5 text-primary" /></div>}
                  </div>
                </li>
              </ul>
            </div>
            ...
            <div className="mt-6">
              ...
              <div className="flex flex-col space-y-2">
                {hobbies.map((hobby, index) => (
                  <div key={index}><span className="capitalize">{hobby.name}</span></div>
                ))}
              </div>
            </div>
          </div>
        ...
        <div className="w-2/3 ml-8">
          ...
          {/* Formations sur la page 1 si aucune page supplémentaire */}
          {extraPages === 0 && educations.length > 0 && (
            <section className="w-full h-fit p-5">
      ...
      {extraPages > 0 && (
        <div className={`page flex flex-col p-16 w-[950px] min-h-[1340px] shadow-lg`} data-theme={theme}>
